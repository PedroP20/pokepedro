...
export default function GoFestPokemonCard({ pokemon, captured, isRaid = false, onToggle, onOpen }: Props) {
  const unavailableArt = !pokemon.pokeApiId;
  return (
    <motion.article layout className={`relative overflow-hidden rounded-2xl border-2 bg-white p-2.5 shadow-sm transition ${captured ? "border-emerald-500 bg-emerald-50/40" : isRaid ? "border-[#FFCB05]" : "border-[#D9D9D9]"}`}>
      ...
      >
        <span className={`absolute left-2 top-2 rounded-full px-1.5 py-0.5 text-[9px] font-stats font-black ${isRaid ? "bg-[#1B4F9C] text-white" : "bg-[#F5F5F5] text-[#1E1E1E]/60"}`}>{isRaid ? "REIDE" : "NATUREZA"}</span>
        <div className="relative mt-2 h-16 w-16 sm:h-20 sm:w-20">
          {pokemon.pokeApiId ? <Image src={`https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/${pokemon.pokeApiId}.png`} alt={pokemon.name} fill sizes="80px" className="object-contain drop-shadow-sm transition group-hover:scale-110" unoptimized /> : <span className="flex h-full w-full items-center justify-center rounded-full bg-slate-100 text-2xl" title="Forma ainda não disponível na fonte de imagens">?</span>}
        </div>
