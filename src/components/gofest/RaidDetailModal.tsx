    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-3 backdrop-blur-sm">
      ...
      <motion.section initial={{ opacity: 0, scale: .94, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: .94, y: 16 }} className="relative z-10 w-full max-w-md overflow-hidden rounded-3xl border border-[#D9D9D9] bg-white shadow-2xl font-navbar">
        <header className="bg-gradient-to-r from-[#1B4F9C] to-[#2A75BB] p-5 text-center text-white">
          ...
          {pokemon.pokeApiId ? <div className="relative mx-auto h-28 w-28"><Image src={`https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/${pokemon.pokeApiId}.png`} alt={pokemon.name} fill sizes="112px" className="object-contain drop-shadow-lg" unoptimized /></div> : <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-white/15 text-3xl">?</div>}
          <p className="mt-1 text-[10px] font-stats font-black tracking-widest text-[#FFCB05]">CHEFE DE MEGARREIDE</p><h2 className="font-heading text-2xl font-black">{pokemon.name}</h2>
        </header>
        <div className="space-y-4 p-5">
          ...
          </> : <div className="rounded-2xl border border-amber-300 bg-amber-50 p-4 text-center"><p className="font-button text-sm font-black text-amber-800">Dados da forma ainda não disponíveis</p><p className="mt-1 text-xs text-amber-900/70">A PokeAPI ainda não possui esta forma. Não exibimos a forma normal como substituta.</p></div>}
