...
export default function GoFestPage() {
  ...
  const dayKeys = useMemo(() => [...new Set([...(selectedDay.superRaids || []), ...selectedDay.slots.flatMap((slot) => [...slot.wild, ...slot.raids])])], [selectedDay]);
  ...
  const renderCards = (keys: string[], raid = false) => keys.map((key) => {
    const pokemon = GOFEST_POKEMON[key];
    return <GoFestPokemonCard key={key} pokemon={pokemon} isRaid={raid} captured={!!captured[key]} onToggle={() => toggleCaptured(key)} onOpen={() => setRaidDetail(pokemon)} />;
  });
